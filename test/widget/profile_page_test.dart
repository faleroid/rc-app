...
class _MockProfilePageState extends State<MockProfilePage> {
  ...
  void _submitChangePassword() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(key: Key('password_success_snackbar'), content: Text('Password berhasil diubah')),
    );
  }
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            // --- User Info ---
            Text(widget.userName, key: const Key('profile_name'), style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
            const SizedBox(height: 4),
            Text(widget.userEmail, key: const Key('profile_email'), style: const TextStyle(fontSize: 14, color: Colors.grey)),
            ...
            Container(
              ...
              decoration: BoxDecoration(
                color: widget.isSubscribed ? Colors.green.shade800 : Colors.grey.shade800,
              ...
              child: Column(
                ...
                children: [
                  Text(
                    widget.isSubscribed ? 'VIP Member Aktif' : 'Belum Berlangganan',
            ...
            OutlinedButton(
              ...
              onPressed: () => setState(() => _showChangePassword = !_showChangePassword),
              child: Text(_showChangePassword ? 'Sembunyikan Ganti Password' : 'Ganti Password'),
            ),
            if (_showChangePassword) ...[
              ...
              TextField(
                ...
                obscureText: true,
                decoration: InputDecoration(labelText: 'Password Lama', errorText: _oldPasswordError),
              ),
              TextField(
                key: const Key('new_password_field'),
                controller: _newPasswordController,
                obscureText: true,
                decoration: InputDecoration(labelText: 'Password Baru', errorText: _newPasswordError),
              ),
              TextField(
                key: const Key('confirm_new_password_field'),
                controller: _confirmNewPasswordController,
                obscureText: true,
                decoration: InputDecoration(labelText: 'Konfirmasi Password Baru', errorText: _confirmError),
              ),
              const SizedBox(height: 12),
              ElevatedButton(
...
void main() {
  group('7. HALAMAN PROFIL WIDGET TESTS', () {
    testWidgets('Nama dan email user ditampilkan dengan benar', (WidgetTester tester) async {
      await tester.pumpWidget(createWidgetUnderTest(
        const MockProfilePage(userName: 'Siti VIP Trader', userEmail: 'siti.vip@ricocapital.id'),
      ));
    ...
    testWidgets('Status subscription aktif ditampilkan', (WidgetTester tester) async {
      await tester.pumpWidget(createWidgetUnderTest(
        const MockProfilePage(userName: 'Siti', userEmail: 'siti@rc.id', isSubscribed: true, expiresAt: '2026-09-01'),
      ));
    ...
    testWidgets('Status subscription tidak aktif ditampilkan', (WidgetTester tester) async {
      await tester.pumpWidget(createWidgetUnderTest(
        const MockProfilePage(userName: 'Andi', userEmail: 'andi@rc.id', isSubscribed: false),
      ));

      expect(find.text('Belum Berlangganan'), findsOneWidget);
    });

    testWidgets('Tanggal expired subscription ditampilkan jika aktif', (WidgetTester tester) async {
      await tester.pumpWidget(createWidgetUnderTest(
        const MockProfilePage(userName: 'Siti', userEmail: 'siti@rc.id', isSubscribed: true, expiresAt: '2026-09-01'),
      ));
    ...
    testWidgets('Form ganti password muncul dengan field password lama & baru', (WidgetTester tester) async {
      await tester.pumpWidget(createWidgetUnderTest(
      ...
      expect(find.byKey(const Key('submit_change_password_btn')), findsOneWidget);
    });

    testWidgets('Validasi form ganti password — field kosong', (WidgetTester tester) async {
      await tester.pumpWidget(createWidgetUnderTest(
        const MockProfilePage(userName: 'Siti', userEmail: 'siti@rc.id'),
      ));
    ...
    testWidgets('Validasi form ganti password — konfirmasi password tidak cocok', (WidgetTester tester) async {
      await tester.pumpWidget(createWidgetUnderTest(
        const MockProfilePage(userName: 'Siti', userEmail: 'siti@rc.id'),
      ));
      ...
      await tester.enterText(find.byKey(const Key('confirm_new_password_field')), 'DifferentPass!');
      await tester.pump();
    ...
    testWidgets('Tombol logout ada dan bisa ditekan', (WidgetTester tester) async {
      await tester.pumpWidget(createWidgetUnderTest(
        const MockProfilePage(userName: 'Siti', userEmail: 'siti@rc.id'),
      ));
    ...
    testWidgets('Setelah logout, navigasi ke LoginScreen dan callback terpanggil', (WidgetTester tester) async {
